...
def den_grow_v(T):
    ...
    # T is undercooling
    a1 = 8.758* (10 ** (-7))#4.6818 * (10 ** (-10))
    a2 = 1.324* (10 ** (-6))#3.1456 * (10 ** (-6))
    devt = a1 * (T ** 2) + a2 * (T ** 3)
    return devt
...
def CA_routine(nx, ny, nz, cell_len, ealist, ea_i, ea_loc_x, ea_loc_y, ea_loc_z, delta_t, rec_step, T_ini=20.0):
    ...
    while t_step >= 0:
        ...
        if t_step_ca == 0:
            if ea_i >= 0:
                ...
                nuc_y = ea_loc_y
                nuc_z = ea_loc_z
                cell_block_ca[nuc_x, nuc_y, nuc_z, 3] = 0.0
                cell_block_ca[nuc_x, nuc_y, nuc_z, 4] = ealist[ea_i,0] * np.pi / 180.0
                cell_block_ca[nuc_x, nuc_y, nuc_z, 2] = 1.0
                cell_block_ca[nuc_x, nuc_y, nuc_z, 5] = ealist[ea_i,1] * np.pi / 180.0
                cell_block_ca[nuc_x, nuc_y, nuc_z, 6] = ealist[ea_i,2] * np.pi / 180.0
            else:
                nuc_x = ea_loc_x
                nuc_y = ea_loc_y
                nuc_z = ea_loc_z
                cell_block_ca[nuc_x, nuc_y, nuc_z, 3] = 0.0
                cell_block_ca[nuc_x, nuc_y, nuc_z, 4] = ealist[:,0] * np.pi / 180.0
                cell_block_ca[nuc_x, nuc_y, nuc_z, 2] = 1.0
                cell_block_ca[nuc_x, nuc_y, nuc_z, 5] = ealist[:,1] * np.pi / 180.0
                cell_block_ca[nuc_x, nuc_y, nuc_z, 6] = ealist[:,2] * np.pi / 180.0
        ...
        if t_step_ca % 5 == 0:
            ...
            if len(nca_train_time) == 0:
                ...
                nca_train_data = np.array(cell_block_ca[..., :, 3:8])[None, ...]
            else:
                nca_train_time.append(t_step_ca)
                nca_train_data = np.vstack((nca_train_data, np.array(cell_block_ca[..., :, 3:8])[None, ...]))

        # judge whether quit the time loop
        if (liq_cell_ca == 0.0) or t_step == 100000:  # if there is no liquid or the time step is too much
            # nca_train_time.append(t_step_ca)
            if len(nca_train_time) != rec_step:
                nca_train_data = np.vstack((nca_train_data, cell_block_ca[..., :, 3:8][None, ...]))
            return nca_train_time, nca_train_data
