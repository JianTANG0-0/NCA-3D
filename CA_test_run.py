            ...
            with VideoWriter('ca_speedup.mp4') as vid_ca:
                while t_step >= 0:
                    ...
                    if t_step_ca == 0:
                        ...
                        #cell_block_ca[..., 0, 3:8] = np.load('./train_data_multi_1535k_3ea.npy')[-1,0,...]
                    ...
                    if t_step_ca == 0:
                        ...
                        x[:, 5:6, ...] = (delta_t*sp_rate/4e-6)/10.

                        device =  torch.device('cpu')# torch.device('cuda' if torch.cuda.is_available() else 'cpu')
                        x[:,:3,...] = x[:,:3,...]*18.0
                        x = torch.cat([F.one_hot(x[:, 0, ...].to(torch.int64), 18).permute(0, 3, 1, 2),F.one_hot(x[:, 1, ...].to(torch.int64), 18).permute(0, 3, 1, 2),F.one_hot(x[:, 2, ...].to(torch.int64), 18).permute(0, 3, 1, 2), x[:, 3:, ...]],
                                      axis=1)
                        x = x.to(device)
                        ca = ca.to(device)
                        start = time.time()
                        ...
                        #melt_filter = torch.argmax(x[:, 20:22, ...], axis=1, keepdims=True)
                        #x = x* melt_filter
                        #x[:, :22, ...] = x[:, :22, ...] * (melt_filter)
                        #x[:, 24:, ...] = x[:, 24:, ...] * (melt_filter)
                        if device.type=='cuda':
                        ...
                        #turn the ont hot code to EA
                        x_train = np.concatenate([np.argmax(x_train[..., :18], axis=-1, keepdims=True) / 18., np.argmax(x_train[..., 18:36], axis=-1, keepdims=True) / 18.,np.argmax(x_train[..., 36:54], axis=-1, keepdims=True) / 18.,x_train[..., 54:]], axis=-1)
