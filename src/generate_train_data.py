...
def gen_data(nx, ny, nz, cell_len, rec_step, delta_t, sample_n, T_type, T_len=1, T_min=15, T_max=25, T_iso=20,ea_type='2D'):
    if T_type == 'noniso':
        for i in range(T_len):
            T_tmp = np.repeat(np.repeat(np.linspace(T_min, T_max, nx)[..., None, None], ny, 1), nz, 2)\
                    + np.repeat(np.repeat(np.linspace(0., 20., nz)[None, None, ...], ny, 1), nx, 0)
            if i == 0:
                T_list = T_tmp[None, ...]
            else:
                rot_gap = 360//T_len
